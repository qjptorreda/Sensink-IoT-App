...
// 2. Start Timer & Update Firebase
    ...
    try {
      ...
      _countdownTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
        if (_remainingTime.inSeconds > 0) {
          ...
          _timeNotifier.value =
              _remainingTime; // Updates UI via ValueListenableBuilder
        } else {
          timer.cancel();
    ...
    try {
      ...
      if (uid != null) {
        await FirebaseFirestore.instance
          ...
          'timerActive': false,
          'shutOffTime':
              FieldValue.delete(), // Cleaner than 'null' for Firestore
        }, SetOptions(merge: true));
      }
    } catch (e) {
  ...
  Widget _getBody() {
    switch (_currentIndex) {
      ...
      case 1:
        return _buildResultsView(); // Now points to the Analytics
      case 2:
        return _buildTimerView();
      case 3:
        return _buildAccountManagementView();
      default:
        return _buildDashboardView();
    }
  }
  ...
  Widget _buildDashboardView() {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        double flowRate = (data['flowRate'] ?? 0.0).toDouble();
          ...
          child: Column(
            ...
            children: [
                  ...
                  children: [
                    Stack(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          onTap: () => _handleFaucetTapFromStream(
                              data['isFaucetOpen'] ?? false),
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              color: _isProcessing
                                  ...
                                  : ((data['isFaucetOpen'] ?? false)
                                      ? Colors.blueAccent
                                      : Colors.redAccent), // Stream-based
                              boxShadow: [
                                BoxShadow(
                                  color: ((data['isFaucetOpen'] ?? false)
                                          ? Colors.blue
                                          : Colors.red)
                                      .withOpacity(0.4),
                                  blurRadius: 30,
                ...
                children: [
                  _buildStatCard(
                    ...
                    () => Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => const PHHistoryScreen())),
                  ),
                  _buildStatCard(
                    "Flow Rate",
                    "${flowRate.toStringAsFixed(1)}\nL/min",
                    Icons.speed_outlined,
                    flowRate > 0 ? Colors.blueAccent : Colors.grey,
                    flowRate > 0 ? "Flowing" : "Idle",
              ...
              _buildWaterLevelCard(
                  waterLevel, () => setState(() => _currentIndex = 1)),
              const SizedBox(height: 20),
              const Text("Quick Controls",
                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              _buildActionTile(Icons.timer, "Set Faucet Timer", "Automate flow",
        ...
        children: [
          ...
          StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
                ...
                .limit(10)
                .snapshots(),
            builder: (context, snapshot) => _buildMiniGraphCard(
                "pH Trend", Colors.blueAccent, snapshot.data?.docs ?? []),
          ),
          const SizedBox(height: 20),
          StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance
  ...
  Widget _buildMiniGraphCard(
      String title, Color color, List<QueryDocumentSnapshot> docs) {
    List<FlSpot> spots = docs
        .asMap()
        .entries
        .map((e) {
          double val =
              (e.value.data() as Map<String, dynamic>)['value']?.toDouble() ??
                  0.0;
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              icon: Icon(
                  _timerActive ? Icons.stop_rounded : Icons.play_arrow_rounded,
                  color: Colors.white),
              label: Text(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor:
                    _timerActive ? Colors.redAccent : Colors.blueAccent,
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 30),
              _buildInfoTile(Icons.phone, "Phone", data['phone'] ?? "Not set",
                  Colors.blue),
              _buildInfoTile(Icons.location_on, "Address",
                  data['address'] ?? "Not set", Colors.orange),
              const SizedBox(height: 30),
              SizedBox(
                width: double.infinity,
...
class PHHistoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFFF8F9FA),
      appBar: AppBar(
        title: const Text("pH Level Analytics",
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          List<FlSpot> spots = logs
              .asMap()
              .entries
              .map((e) {
                double val = (e.value.data() as Map<String, dynamic>)['value']
                        ?.toDouble() ??
                    0.0;
                return FlSpot(e.key.toDouble(), val);
              })
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                            ...
                            child:
                                Icon(Icons.science, color: _getColor(value))),
                        title: Text("pH: ${value.toStringAsFixed(1)}",
                            style:
                                const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle:
                            Text(DateFormat('yMMMd').add_jm().format(date)),
                        trailing: Text(_getStatus(value),
                            style: TextStyle(
  ...
  Color _getColor(double val) =>
      (val < 6.5 || val > 8.5) ? Colors.orange : Colors.green;
  String _getStatus(double val) => (val < 6.5)
      ? "ACIDIC"
      : (val > 8.5)
          ? "ALKALINE"
          : "OPTIMAL";
